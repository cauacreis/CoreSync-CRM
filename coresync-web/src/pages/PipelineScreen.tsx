  ...
  return (
    <div className="flex h-full w-full flex-col p-8 overflow-hidden bg-zinc-950">
      ...
      <div className="mb-8 flex items-center justify-between shrink-0 flex-wrap gap-4">
        <h1 className="text-5xl font-black uppercase tracking-tighter text-zinc-100">{t('pipeline.title')}</h1>
        <div className="flex gap-4 flex-wrap">
          ...
          <button
            ...
            className="border-4 border-zinc-100 bg-zinc-400 px-4 py-2 font-black text-zinc-950 transition-transform active:translate-x-1 active:translate-y-1 hover:bg-zinc-300 shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]"
            title="Configurações"
          >
            ⚙️
          </button>
      ...
      <div className="relative flex h-full min-h-0 w-full flex-col group">
        ...
        <button
          onClick={scrollLeft}
          className="absolute left-0 top-1/2 z-10 -translate-x-4 -translate-y-1/2 border-4 border-zinc-100 bg-lime-400 px-4 py-6 text-2xl font-black text-zinc-950 opacity-0 shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] transition-all hover:bg-lime-300 active:translate-x-[-12px] active:translate-y-1 group-hover:opacity-100"
        >
          {'<'}
        </button>
        ...
        <button
          onClick={scrollRight}
          className="absolute right-0 top-1/2 z-10 translate-x-4 -translate-y-1/2 border-4 border-zinc-100 bg-lime-400 px-4 py-6 text-2xl font-black text-zinc-950 opacity-0 shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] transition-all hover:bg-lime-300 active:translate-x-5 active:translate-y-1 group-hover:opacity-100"
        >
          {'>'}
        </button>
        ...
        >
          {STATUSES.map((status) => (
            <div key={status} className="flex w-[320px] shrink-0 flex-col border-4 border-zinc-800 bg-zinc-900 p-4">
                ...
                {leads
                  ...
                  .map((lead) => (
                    ...
                    >
                      ...
                      <div className="text-lg font-black text-white mt-1">{formatCurrency(lead.estimatedValue)}</div>
