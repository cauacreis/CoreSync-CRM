    ...
    <div className="flex h-full w-full flex-col p-8 overflow-y-auto">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-md border-4 border-zinc-100 bg-zinc-900 p-8 shadow-[12px_12px_0px_0px_rgba(163,230,53,1)]">
            <h2 className="mb-6 text-3xl font-black uppercase text-white">Cadastrar Produto</h2>
            <form onSubmit={handleSave} className="flex flex-col gap-4">
              <div>
                <label className="mb-2 block font-bold uppercase text-zinc-400">Nome</label>
                <input
                  required
                  type="text"
                  value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  className="w-full border-4 border-zinc-100 bg-zinc-800 px-4 py-2 font-bold text-white outline-none focus:border-lime-400"
                />
              </div>
              <div>
                <label className="mb-2 block font-bold uppercase text-zinc-400">Descrição</label>
                <textarea
              ...
              <div>
                <label className="mb-2 block font-bold uppercase text-zinc-400">Preço (USD)</label>
                <input
                  required
                  type="number"
                  step="0.01"
              ...
              <div className="mt-4 flex gap-4">
                <button
                  ...
                  className="flex-1 border-4 border-zinc-100 bg-zinc-500 py-3 font-bold text-white transition-transform active:translate-x-1 active:translate-y-1 hover:bg-zinc-400"
                >
                  CANCELAR
                </button>
                <button
                  type="submit"
                  className="flex-1 border-4 border-zinc-100 bg-lime-400 py-3 font-bold text-zinc-950 transition-transform active:translate-x-1 active:translate-y-1 hover:bg-lime-300"
                >
                  SALVAR
                </button>
              </div>
